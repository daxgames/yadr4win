...
if exist "%CMDER_ROOT%\vendor\git-for-windows" (
  ...
  set "zshCmd=%CMDER_ROOT%\vendor\git-for-windows\usr\bin\zsh.exe"
) else if exist "%ProgramFiles%\git" (
  set "PATH=%ProgramFiles%\git\usr\bin;%PATH%"
  set "gitCmd=%ProgramFiles%\git\git-cmd.exe"
  set "zshCmd=%ProgramFiles%\git\usr\bin\zsh.exe"
  if not exist "%ProgramFiles%\git\etc\profile.d\cmder_exinit.sh" (
    echo Run 'mklink "%ProgramFiles%\git\etc\profile.d\cmder_exinit.sh" "%CMDER_ROOT%\vendor\cmder_exinit"' in 'cmd::Cmder as Admin' to use Cmder with external Git zsh
    echo.
    echo or
    echo.
  ...
  set "zshCmd=%ProgramFiles(x86)%\git\usr\bin\zsh.exe"
  if not exist "%ProgramFiles(x86)%\git\etc\profile.d\cmder_exinit.sh" (
    echo Run 'mklink "%ProgramFiles^(x86^)%\git\etc\profile.d\cmder_exinit.sh" "%CMDER_ROOT%\vendor\cmder_exinit"' in 'cmd::Cmder as Admin' to use Cmder with external Git zsh
    echo.
    echo or
    echo.
...
if defined ConEmuDir (
  "%gitCmd%" --no-cd %gitCommand% "%zshCmd%" --login -i
) else (
  "%zshCmd%" --login -i
)
